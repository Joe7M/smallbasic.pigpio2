...
int _read_config(ads1x15_p s)
{
   ...
   // set low compare register
   buf[0] = COMPARE_LOW_REG;
   write(s->i2ch, buf, 1); 
   read(s->i2ch, buf, 2);
   s->compare_low = (buf[0] << 8) | buf[1];

  // set high compare register
...
int _update_config(ads1x15_p s)
{
   ...
   s->configH = ((1 << 7) | (s->channel << 4) | (s->gain << 1) | s->single_shot);
...
int CMD_Open(int argc, slib_par_t *params, var_t *retval)
{
  ...
  strcpy(device,text);
  uint8_t Address = get_param_int(argc, params, 1, 0x48);
  uint8_t Version = get_param_int(argc, params, 2, 1);

  adc = (ads1x15_p)calloc(1, sizeof(ads1x15_t));
  if (adc == NULL)
  {
    v_setstr(retval, "Cannot allocate memory.");
    return (0);
  }

  adc->i2ch = open(device, O_RDWR);
  if (adc->i2ch < 0)
  {
    free(adc);
  ...
  int16_t rc = ioctl(adc->i2ch, I2C_SLAVE, Address);
  if (rc < 0)
  {
    v_setstr(retval, "Error opening ADS1x15");
    close(adc->i2ch);
    free(adc);
    return(0);
  }
  ...
  _read_config(adc);
  ADS1X15_alert_never(adc); // switch off ALERT/RDY pin.
...
{
    float VoltageRange = get_param_num(argc, params, 0, 6.144);
...
{
    int SampelRate = get_param_int(argc, params, 0, 128);
